...
void slope_shms()
{
  ...
  for (int ip = 0; ip<12; ip++)

    {
      ...
      time = Form("H.dc.%s.time<190.0",plane[ip].c_str());
      
      drift_dist[ip] = new TH1F(Form("drift_dist_%s", plane[ip].c_str()),  "", 100, -0.01, 0.6 );
      t->Draw(Form("H.dc.%s.dist>>drift_dist_%s", plane[ip].c_str(), plane[ip].c_str()), Form("H.dc.%s.time<190.0",plane[ip].c_str()),Form("P.dc.%s.nhit == 1",plane[ip].c_str()));
