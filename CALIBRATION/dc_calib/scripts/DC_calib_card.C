...
void DC_calib::SetPlaneNames()
{
  ...
  if(spec=="SHMS")
  ...
  else if(spec=="HMS")
    {
      ...
      //Define number of CARDS per plane
      plane_cards[0] = 6;
      plane_cards[1] = 6;
      plane_cards[2] = 7;
      plane_cards[3] = 7;
  ...
  if (num_evts > nentries)
    {
      ...
      cout << "Setting number of events = " << nentries << endl;
      num_evts = nentries;
  ...
  for(Long64_t i=0; i<num_evts; i++)
    {
	  ...
	  for(Int_t j = 0, k = 0; j < ndata_time[ip], k < ndata_wirenum[ip]; j++, k++)    
		...
		// Bool_t card1_1u1 = wire >=1 && wire <=10;
		// Bool_t card2_1u1 = wire>=11 && wire <=26;

		// if (ip == 0 && card1_1u1)
		//   {
		//     card1_dt1x1->Fill(drift_time[ip][j]);
...
void DC_calib::GetTwentyPercent_Card()
{
  ...
  binContentMax = binContentLow = binContentHigh = 0.0;
  ...
  for (int ip = 0; ip<NPLANES; ip++)
      ...
      for (card = 0; card < plane_cards[ip]; card++)
	{
	  ...
	  binSearchHigh =  card_hist[ip][card].GetMaximumBin();
	  wireBinContentMax[ip][card] = card_hist[ip][card].GetMaximum();
	  wireBinContentLow[ip][card]  = wireBinContentMax[ip][card]*0.20;
	  wireBinContentHigh[ip][card] = wireBinContentMax[ip][card]*0.80;
	  card_hist[ip][card].GetBinWithContent(wireBinContentLow[ip][card],  binValLow,  binSearchLow, binSearchHigh, binDiffThresh); 
	  card_hist[ip][card].GetBinWithContent(wireBinContentHigh[ip][card], binValHigh, binSearchLow, binSearchHigh, binDiffThresh); 
	  wireBinLow[ip][card]  = binValLow;
	  wireBinHigh[ip][card] = binValHigh;
	  wireFitRangeLow[ip][card]  = card_hist[ip][card].GetBinCenter(wireBinLow[ip][card]);
	  wireFitRangeHigh[ip][card] = card_hist[ip][card].GetBinCenter(wireBinHigh[ip][card]);

	  if((ip == 5 && card ==5) || (ip == 10 && card==5) || (ip == 11 && card == 5) || (ip ==4 && card == 5)||(ip==11 &&card == 0)||(ip==9 && card==5))
	     {
	       wireBinContentLow[ip][card]  = wireBinContentMax[ip][card]*0.15;
	       wireBinContentHigh[ip][card] = wireBinContentMax[ip][card]*0.60;
	       card_hist[ip][card].GetBinWithContent(wireBinContentLow[ip][card],  binValLow,  binSearchLow, binSearchHigh, binDiffThresh); 
	       card_hist[ip][card].GetBinWithContent(wireBinContentHigh[ip][card], binValHigh, binSearchLow, binSearchHigh, binDiffThresh); 
	       wireBinLow[ip][card]  = binValLow;
	       wireBinHigh[ip][card] = binValHigh;
	   ...
	   cout << "wire 20% :"<< wireBinContentLow[ip][card]<<endl;
	   cout << "wire 80% :"<<  wireBinContentHigh[ip][card]<<endl;
	   cout <<"bin low value :"<<binValLow<<"\t"<<"high b invalue: "<<binValHigh<<endl;
	   cout <<"low range :"<<  wireFitRangeLow[ip][card]<<"\t"<< "high range :" << wireFitRangeHigh[ip][card]<<endl;
	   cout <<"***********************************************"<<endl;
	     }//end of if 

	} //end loop per cards
    }// end loop for NPLANES
}// end loop for class GetTwentyPercent_card
...
void DC_calib::FitCardDriftTime()
{
  ...
  for (Int_t ip = 0; ip < NPLANES; ip++)
      ...
      for (card = 0; card < plane_cards[ip]; card++)
	{
	  ...
	  //Get Bin no. associated with the reference time
	  // binx = card_hist[ip][card].GetXaxis()->FindBin(ref_time[ip][card]);
	  
	  // //Get time corresponding to bin (fit range) 
	  // time_init = card_hist[ip][card].GetXaxis()->GetBinCenter(binx - 10); //choose bin range over which to fit
	  // time_final = card_hist[ip][card].GetXaxis()->GetBinCenter(binx + 10); 
	  
	  //Create Fit Function
	  // if (ip==6 && card== 0)
	  //   {
	  //     cout << "******FitRange Low: " << wireFitRangeLow[ip][card] << endl;
	  //     cout << "******FitRange High: " << wireFitRangeHigh[ip][card] << endl;
	  //   }
	  tZero_fit = new TF1("tZero_fit", "[0]*x + [1]", wireFitRangeLow[ip][card], wireFitRangeHigh[ip][card]);
	  
	  // cout <<"The tZero Value :"<< tZero_fit<<endl;
	  //Set Parameter Names and Values
	  tZero_fit->SetParName(0, "slope");
	  tZero_fit->SetParName(1, "y-int");
	  tZero_fit->SetParameter(0, 1.0);
	  ...
	  if (abs(-y_int/m) < std_dev*5.0 && m > 0.0 && entries_card[ip][card]>max_wire_entry)
	    {
	      ...
	      // t_zero[ip][card] = - y_int/m ;
	      // t_zero_err[ip][card] = sqrt(y_int_err*y_int_err/(m*m) + y_int*y_int*m_err*m_err/(m*m*m*m) );
...
void DC_calib::WriteToFile(Int_t debug = 0)
{
  ...
  main_dir = out_file->mkdir("uncorr_wire_times");
	...
	for (wire = 0; wire < nwires[ip]; wire++)
      ...
      //--------write uncorrected CARD drift times histos to FILE--------ABISHEK  
...
//-------Write Fitted Card Drift Time histos to FILE----------------------------
  ...
  main_dir = out_file->mkdir("fitted_card_drift_times");
    ...
    //-----Write 'tzero-per-wire' values to a TEXT FILE--------------------
...
//_______________________________________________________________________
void DC_calib::ApplyTZeroCorrectionPerCard()
{
...

{
  ...
  out_txtFILE << spectre+Form("drift1stbin=%f", t_offset_firstbin + 1.0) << "\n";
...
void DC_calib::GetCard()
{
 ...
 wire_max[7][5]=96;
    ...
    {
      wire_min[2][0]=1,      wire_max[2][0]=16;//1x1
      wire_min[2][1]=17,     wire_max[2][1]=33;
      wire_min[2][2]=34,     wire_max[2][2]=49;
      wire_min[2][3]=50,     wire_max[2][3]=64;
      wire_min[2][4]=65,     wire_max[2][4]=79;
      //*************************************
      wire_min[3][0]=1,      wire_max[3][0]=15;//1x2
      ...
      //**************************************
      wire_min[0][0]=1,      wire_max[0][0]=15;//1u1
      ...
      //**************************************
      wire_min[1][0]=1,      wire_max[1][0]=12;//1u2
      ...
      //******************************************
      wire_min[5][0]=1,      wire_max[5][0]=15;//1v1
      wire_min[5][1]=16,     wire_max[5][1]=31;
      wire_min[5][2]=32,     wire_max[5][2]=47;
      wire_min[5][3]=48,     wire_max[5][3]=63;
      wire_min[5][4]=65,     wire_max[5][4]=79;
      wire_min[5][5]=84,     wire_max[5][5]=95;
      wire_min[5][6]=96,     wire_max[5][6]=107;
      //****************************************
      wire_min[4][0]=1,      wire_max[4][0]=12;//1v2
      wire_min[4][1]=13,     wire_max[4][1]=28;
      wire_min[4][2]=29,     wire_max[4][2]=44;
      wire_min[4][3]=45,     wire_max[4][3]=60;
      wire_min[4][4]=61,     wire_max[4][4]=76;
      wire_min[4][5]=77,     wire_max[4][5]=92;
      wire_min[4][6]=93,     wire_max[4][6]=107;
      //****************************************
      wire_min[9][0]=1,      wire_max[9][0]=16;//2x1
      wire_min[9][1]=17,     wire_max[9][1]=33;
      wire_min[9][2]=34,     wire_max[9][2]=49;
      wire_min[9][3]=50,     wire_max[9][3]=64;
      wire_min[9][4]=65,     wire_max[9][4]=79;
      //*****************************************
      wire_min[8][0]=1,      wire_max[8][0]=15;//2x2
      ...
      //**************************************
      wire_min[11][0]=1,      wire_max[11][0]=15;//2u1
      ...
      //**************************************
      wire_min[10][0]=1,      wire_max[10][0]=12;//2u2
      ...
      //******************************************
      wire_min[6][0]=1,      wire_max[6][0]=15;//2v1
      wire_min[6][1]=16,     wire_max[6][1]=31;
      wire_min[6][2]=32,     wire_max[6][2]=47;
      wire_min[6][3]=48,     wire_max[6][3]=63;
      wire_min[6][4]=65,     wire_max[6][4]=79;
      wire_min[6][5]=84,     wire_max[6][5]=95;
      wire_min[6][6]=96,     wire_max[6][6]=107;
      //****************************************
      wire_min[7][0]=1,      wire_max[7][0]=12;//2v2
      wire_min[7][1]=13,     wire_max[7][1]=28;
      wire_min[7][2]=29,     wire_max[7][2]=44;
      wire_min[7][3]=45,     wire_max[7][3]=60;
      wire_min[7][4]=61,     wire_max[7][4]=76;
      wire_min[7][5]=77,     wire_max[7][5]=92;
      wire_min[7][6]=93,     wire_max[7][6]=107;
      //****************************************
	 ...
	 for(Int_t j = 0; j<ndata_time[ip]; j++)
	   {
	     ...
	     //--------ADD Uncorrecte plane drift times-------------
	       //Fill uncorrected plane drift times  (from: get_pdc_time_histo.C )
		if (ip== 0 && wire > 80)
		  {
		    plane_dt[ip].Fill(drift_time[ip][j] - offset); 
		    dt_vs_wire[ip].Fill(wire_num[ip][j], drift_time[ip][j] - offset);
		  ...
		  {
		    plane_dt[ip].Fill(drift_time[ip][j]); 
		    dt_vs_wire[ip].Fill(wire_num[ip][j], drift_time[ip][j]);
