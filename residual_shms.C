...
#define NPLANES 12
void residual_shms(Int_t runNo, Int_t eventNo)

{
  ...
  TFile *f =  new TFile(Form("ROOTfiles/shms_replay_production_all_%d_%d.root",runNo, eventNo));
  TTree *t = (TTree*)f->Get("T");
  
  TFile *output = new TFile(Form("residual_%d_%d.root", runNo, eventNo), "RECREATE");
