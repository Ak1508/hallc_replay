...
runReportMon="./readout_shms.py ${runNum} ${numEvents}"
runReportTable="./table_1.py ${runNum} ${numEvents}"

# Define report monitoring directroy
reportMonDir="UTIL/REPORT_MON/${SPEC}/"
#reportMonTable="REPORT_MONITOR/{SPEC}/"
...
pathReportTable="../../../REPORT/table.txt"
...
# Start analysis and monitoring plots.
{
  ...
  sleep 2
  echo ":=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:="
  echo "Generating report monitoring TABLE  file ${SPEC}"
  echo ""
  echo ":=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:="
 ...
 # cd ${reportMonTable}
  eval ${runReportTable}
  #mv "${outFileTable}.csv" ${pathReportTable}
  emacs ../../MON_REPORT/REPORT/table.txt
  sleep 2
