...
void CTcoinTime(Int_t runNumber,  TString shms_particle, Int_t analyzedEvents)
{
    TString fileName = "/net/cdaq/cdaql3data/cdaq/hallc-online/ROOTfiles/";
	...
	if(!(SHMScorrCoinTimeROC1 >-7 && SHMScorrCoinTimeROC1<-2.5))
	  continue;
	//------ Fill the hostograms -------
...
anvas *c1 = new TCanvas("c1","Beta Distribution");
    c1->Divide(2,2);
    c1->cd(1);
    h1PgtrBetaCalc->Draw();
    c1->cd(2);    
    ...
    h1PcointimeROC1->GetXaxis()->SetRange(-7,-2.5);
