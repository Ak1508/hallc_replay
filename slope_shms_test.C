// slope for SHMS
void slope_shms_test(Int_t runNo)
{
  ...
  TFile *output = new TFile(Form("output_test_norm_%d.root", runNo), "RECREATE");
  ...
  for (int ip = 0; ip<12; ip++)
    {
      ...
      //  hnorm[ip] = new TH1F (Form("norm_hist_%s", plane[ip].c_str()),  "", 100, -0.01, 0.6 );
